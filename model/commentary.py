...
class Commentary:
    ...
    def get_player_object_from_short_name(self, name):
        if name in self.player_objects:
            return self.player_objects[name]
        for player_id in self.squad.keys():
            player = self.squad[player_id]
            if name in player.short_names:
                self.player_objects[name] = player
                return player
    ...
    def get_head_to_head_data(self):
        ...
        for ball_commentary in self.commentary_data:
            ...
            if len(players) < 2:
                ...
                continue
            batsman = self.get_player_object_from_short_name(players[1].strip())
            bowler = self.get_player_object_from_short_name(players[0].strip())
    ...
    def __get_head_to_head_object(self, batsman, bowler):
        if batsman.name in self.head_to_head_object_cache:
            if bowler.name not in self.head_to_head_object_cache[batsman.name]:
                self.head_to_head_object_cache[batsman.name][bowler.name] = HeadToHead(batsman, bowler)
        else:
            self.head_to_head_object_cache[batsman.name] = {}
            self.head_to_head_object_cache[batsman.name][bowler.name] = HeadToHead(batsman, bowler)
        return self.head_to_head_object_cache[batsman.name].get(bowler.name)
